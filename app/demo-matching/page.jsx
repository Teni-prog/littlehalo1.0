...
export default function DemoMatchingPage() {
  ...
  const handleFindMatch = (parent) => {
    ...
    // Convert your parent structure into new matching format
    const parentConfig = {
      must: {
        specialNeeds: parent.preferences.specialNeedsRequired === true,
        background_check_status: "approved",
      },
      weights: {
        price: 0.3,
    ...
    // Transform dummySitters into algorithm-compatible format
    const formattedSitters = dummySitters.map((sitter) => ({
      id: sitter.id,
      price: sitter.hourly_rate,
      rating: sitter.rating,
      experience: sitter.years_experience,
      languageMatch: sitter.languages?.includes(
        parent.preferences.preferredLanguage,
      )
        ? 1
        : 0,
      specialNeeds: sitter.special_needs_experience === true,
      background_check_status: sitter.background_check_status,
    }));

    const results = matchSitters(formattedSitters, parentConfig);
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-bold text-center mb-8">
          Matching Algorithm Demo (MAUT + Pareto)
        </h1>

        {/* PARENTS */}
        <div className="grid grid-cols-2 gap-8 mb-10">
          <div>
            <h2 className="text-2xl font-semibold mb-4">Parents</h2>
            ...
            {parents.map((parent) => (
              ...
              >
                ...
                <p className="text-sm text-gray-600">
                  📍 {parent.preferences.city}
                </p>
                <p className="text-sm text-gray-600">
                  💰 Budget: ${parent.preferences.maxBudget}/hr
                </p>
                <p className="text-sm text-gray-600">
                  🗣️ {parent.preferences.preferredLanguage}
                </p>
          ...
          <div>
            ...
            {!matchResult && (
              <div className="bg-white p-6 rounded shadow text-gray-500">
                Select a parent to run matching.
              </div>
            )}
            ...
            {matchResult?.bestMatch && (
              <div className="bg-green-100 p-4 rounded border border-green-500 mb-6">
                <h3 className="font-bold text-lg">Best Match</h3>
                <p className="text-sm mt-2">
                  🧑{" "}
                  {matchResult.bestMatch.name ||
                    `Sitter ${matchResult.bestMatch.id}`}
                </p>
                <p className="text-sm">
                  ⭐ Score: {matchResult.bestMatch.score?.toFixed(3)}
                </p>
              </div>
            )}

            {matchResult?.allResults?.length > 0 && (
              <div className="space-y-4">
                {matchResult.allResults.map((sitter, idx) => (
                  <div
                    key={sitter.id}
                    className={`p-4 rounded border ${
                      sitter.id === matchResult.bestMatch?.id
                  ...
                  >
                    <div className="flex justify-between">
                      <h4 className="font-semibold">
                        {idx + 1}. {sitter.name || `Sitter ${sitter.id}`}
                      </h4>
                      <span className="text-sm font-medium text-blue-600">
                        {sitter.score?.toFixed(3)}
                      </span>
                    </div>
                    ...
                    <div className="mt-2 text-sm text-gray-600">
                      ...
                      <p>📈 Experience: {sitter.experience} yrs</p>
                      <p>🗣️ Language Match: {sitter.languageMatch}</p>
                    </div>
                  </div>
                ))}
              </div>
