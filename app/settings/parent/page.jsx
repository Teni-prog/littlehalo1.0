...
function CountrySelect({ value, onChange }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className={value ? "text-gray-900" : "text-gray-400"}>{value || "Select country…"}</span>
      ...
      <table className="w-full text-sm border-separate border-spacing-1">
        <thead>
          <tr>
            <th className="w-16" />
            {SLOTS.map(s => (
              <th key={s} className="text-xs font-semibold text-gray-500 capitalize text-center pb-1">{s}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {DAYS.map(day => (
            <tr key={day}>
              <td className="text-xs font-semibold text-gray-600 pr-2">{day}</td>
              {SLOTS.map(slot => (
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Your public-facing information shown to sitters.</p>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          {avatarPreview && (
            <p className="text-xs text-gray-400 mt-0.5">Preview — save to apply</p>
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Full name" required error={errors.name}>
          <input value={name} onChange={e => setName(e.target.value)} className={inputCls(errors.name)} placeholder="Jane Doe" />
        </Field>
      ...
      <Field label="Location" hint="Select your province and city. Neighbourhood is optional and auto-sets your coordinates.">
      ...
      <Field label="Short note to sitters" hint={`${note.length}/300`}>
        <textarea
          value={note}
          onChange={e => e.target.value.length <= 300 && setNote(e.target.value)}
          rows={3}
          placeholder="Tell sitters about your family, routines, or anything helpful…"
...
function ChildForm({ initial = EMPTY_CHILD, onSave, onCancel, saving }) {
  ...
  function submit(e) {
    ...
    if (!data.date_of_birth) errs.dob = "Date of birth is required.";
    if (Object.keys(errs).length) { setErrors(errs); return; }
    ...
    <form onSubmit={submit} className="bg-gray-50 rounded-2xl border border-gray-100 p-5 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Child's name" required error={errors.name}>
          <input value={data.name} onChange={e => set("name", e.target.value)} className={inputCls(errors.name)} placeholder="Emma" />
        </Field>
      ...
      <Field label="Allergies / medical notes">
        <textarea
          ...
          rows={2}
          placeholder="Peanut allergy, EpiPen in red bag…"
          className={`${inputCls()} resize-none`}
        />
      </Field>
      ...
      <div className="flex gap-2 justify-end">
        ...
        <SaveBtn saving={saving} label={initial.id ? "Save child" : "Add child"} />
      </div>
    </form>
  );
}
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-5">
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          {age != null && <p className="text-sm text-gray-500">{age} years old</p>}
        </div>
      ...
      {child.special_needs?.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-2">
          {child.special_needs.map(n => (
            <span key={n} className="text-xs bg-teal-50 text-teal-700 px-2.5 py-1 rounded-full">{n}</span>
          ))}
        </div>
      )}
      {child.medical_notes && (
      ...
      {confirmDelete && (
        <div className="mt-3 pt-3 border-t border-gray-100">
          <p className="text-sm text-red-600 font-medium mb-2">
            Remove {child.name}? This cannot be undone.
          </p>
          <div className="flex gap-2">
            <button
              onClick={onDelete}
...
function ChildrenSection({ userId, showToast }) {
  ...
  async function handleSave(formData) {
    ...
    try {
      ...
      showToast(err.message || "Failed to save child", "error");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id) {
    ...
    if (error) { showToast("Failed to remove child", "error"); return; }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage the children in your care.</p>
        </div>
        {editing !== "new" && (
          <button
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Help sitters understand what you're looking for.</p>
      </div>

      <Field label="Budget range ($/hr)">
      ...
      <Field label="Preferred availability" hint="Select the times that work best for your family">
        <div className="mt-1">
    ...
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-red-100 p-6">
        ...
        <p className="text-sm text-gray-500 mb-4">
          Deleting your account is permanent. All your data, bookings history, and children profiles will be removed.
        </p>
        <button
