...
const SPECIAL_NEED_OPTION_KEYS = {
  Autism: "autism",
  ADHD: "adhd",
  Sensory: "sensory",
        ...
        <div className="p-6 space-y-8 flex-1">
          ...
          <div>
            ...
            <div className="flex justify-between text-xs text-gray-400 mt-1">
              <span>{t("filters.zero")}</span>
              {filters.experience > 0 && (
                <span className="text-[#ff6b6b] font-semibold">
                  {t("filters.yearsPlus", { count: filters.experience })}
                </span>
              )}
              <span>{t("filters.maxExperienceLabel")}</span>
            </div>
          </div>
                ...
                {ALL_SPECIAL_NEEDS.map((need) => (
                  <label key={need} className="flex items-center gap-2.5 cursor-pointer group">
                    <input
                      ...
                      style={{ accentColor: "#ff6b6b" }}
                    />
                    <span className="text-sm text-gray-700 group-hover:text-gray-900">{t(`filters.specialNeedsOptions.${SPECIAL_NEED_OPTION_KEYS[need]}`)}</span>
                  </label>
                ))}
              </div>
            )}
                ...
                {ALL_LANGUAGES.map((lang) => (
                  <label key={lang} className="flex items-center gap-2.5 cursor-pointer group">
                    <input
                      ...
                      style={{ accentColor: "#ff6b6b" }}
                    />
                    <span className="text-sm text-gray-700 group-hover:text-gray-900">{t(`filters.languageOptions.${LANGUAGE_OPTION_KEYS[lang]}`)}</span>
                  </label>
                ))}
              </div>
            )}
              ...
              {["weekdays", "weekends"].map((value) => (
                ...
                >
                  ...
                  <span className="text-sm text-gray-700 group-hover:text-gray-900">
                    {t(`filters.availabilityOptions.${value}`)}
                  </span>
                </label>
              ))}
            </div>
                  ...
                  <div key={c.key}>
                    <div className="flex justify-between items-center mb-1">
                      <label className="text-xs text-gray-600">
                        {c.icon} {t(`filters.criteria.${c.key}`)}
                      </label>
