...
export default function MatchedSitterCard({ sitter }) {
  const t = useTranslations("searchMatchingButton");
  const topsisScore = sitter.score ?? null;
  const scoreLabel =
    topsisScore !== null
      ? t("scoreMatch", { percent: (topsisScore * 100).toFixed(0) })
      : null;
    ...
    <div className="bg-white border border-gray-100 rounded-2xl p-6 hover:shadow-lg transition-shadow">
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-4">
          <Image
            ...
            alt={sitter.name || t("sitterFallbackAlt")}
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 text-sm text-gray-600">
          ...
          <span>{t("reviewsCount", { count: sitter.reviews })}</span>
        </div>

        <div className="flex items-center gap-2 text-sm text-gray-600">
          <Briefcase className="w-4 h-4" />
          <span>{t("experienceYears", { count: sitter.experience })}</span>
        </div>

        <div className="flex items-center gap-2 text-sm text-gray-600">
          <Briefcase className="w-4 h-4" />
          <span>{t("hourlyRate", { rate: sitter.hourly_rate })}</span>
        </div>
