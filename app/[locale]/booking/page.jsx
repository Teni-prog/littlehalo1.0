...
function formatSlotHour(h, t) {
  const n = parseInt(h, 10);
  if (n === 0) return `12 ${t("time.am")}`;
  if (n < 12) return `${n} ${t("time.am")}`;
  if (n === 12) return `12 ${t("time.pm")}`;
...
function BookingPageContent() {
    ...
    setBookingDetails((prev) => {
      ...
      if (selectedAdventuresFromLibrary.length > 1) {
        setLastAddedAdventureTitle(
          t("adventureCountLabel", {
            count: selectedAdventuresFromLibrary.length,
          }),
        );
  ...
  }, [sitterId, selectedAdventureFromLibrary, selectedAdventuresFromLibrary, t]);
  ...
  const handleSlotClick = (hour) => {
    ...
    if (status === "booked") {
      showSlotToast(t("toast.booked"));
      return;
    }
    if (status === "unavailable") {
      showSlotToast(t("toast.unavailable"));
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-2xl shadow-sm p-10 max-w-md w-full text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">{t("accessRestricted.title")}</h2>
          <p className="text-gray-600 mb-8">{t("accessRestricted.message")}</p>
          <Link href="/profile/Sitter">
            <Button className="bg-teal-500 hover:bg-teal-600 text-white font-semibold px-8 py-3 cursor-pointer">
              {t("accessRestricted.dashboardButton")}
            </Button>
          </Link>
        </div>
      </div>
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-700">
                    {t("banner.selected", { title: lastAddedAdventureTitle })}
                  </p>
                </div>
              </div>
              <button
                      ...
                      <div className="space-y-3">
                        ...
                        {!bookingDetails.startTime ? (
                          ...
                          <div className="flex items-center justify-between rounded-lg bg-teal-50 border border-teal-200 px-3 py-2">
                            <span className="flex items-center gap-2 text-sm font-medium text-teal-800">
                              ...
                              {t("dateTime.startSelected", {
                                time: formatSlotHour(bookingDetails.startTime.split(":")[0], t),
                              })}
                            </span>
                            <button
                              type="button"
                          ...
                          <div className="flex items-center justify-between rounded-lg bg-teal-50 border border-teal-200 px-3 py-2">
                            <span className="flex items-center gap-2 text-sm font-medium text-teal-800">
                              <Clock className="h-4 w-4 shrink-0" />
                              {t("dateTime.selectedRange", {
                                start: formatSlotHour(bookingDetails.startTime.split(":")[0], t),
                                end: formatSlotHour(bookingDetails.endTime.split(":")[0], t),
                                hours: parseInt(bookingDetails.endTime, 10) - parseInt(bookingDetails.startTime, 10),
                              })}
                            </span>
                            <button
                              type="button"
                          ...
                          {BOOKING_SLOTS.map((hour) => {
                            ...
                            return (
                              <button
                                ...
                                title={
                                  status === "booked"
                                    ? t("dateTime.slotTooltipBooked")
                                    : status === "unavailable"
                                    ? t("dateTime.slotTooltipUnavailable")
                                    : ""
                                }
                              >
                                {formatSlotHour(hour, t)}
                              </button>
                            );
                          })}
                        </div>
                        ...
                        {rangeHasUnavailableHours && (
                          <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
                            ...
                            {t("dateTime.partialAvailabilityWarning")}
                          </div>
                        )}
                ...
                <Card>
                  <CardHeader>
                    <span className="flex justify-between">
                      ...
                      >
                        <i className="text-sm text-red-500">{t("adventures.browseMore")}</i>
                      </Link>
                    </span>
                  </CardHeader>
                  <CardContent>
                    ...
                    <div className="space-y-3">
                            ...
                            <div className="flex-1">
                              <div className="flex justify-between items-start">
                                ...
                                <span className="text-[#ff6b6b] font-semibold">
                                  {t("adventures.priceValue", { price: adventure.price })}
                                </span>
                              </div>
                              ...
                              <p className="text-xs text-gray-400 mt-1">
                                {t("adventures.durationLabel", { duration: adventure.duration })}
                              </p>
                            </div>
                            <Button
                              type="button"
                      ...
                      {selectedAdventureRecords.length > 0 && (
                        <p className="text-xs text-gray-500">
                          {t("adventures.showingSelectedPrefix")}{" "}
                          <strong>{t("adventures.browseMore")}</strong>{" "}
                          {t("adventures.showingSelectedSuffix")}
                        </p>
                      )}
                    </div>
                  </CardContent>
                ...
                <CardContent className="space-y-4">
                  ...
                  {selectedSitter && (
                    <div className="pb-4 border-b">
                      <p className="text-sm text-gray-500 mb-1">{t("summary.sitterLabel")}</p>
                  ...
                  {/* Date & Time Summary */}
                  <div className="pb-4 border-b">
                    <p className="text-sm text-gray-500 mb-2">{t("summary.dateTimeLabel")}</p>
                    {bookingDetails.date ? (
                      ...
                      <p className="text-gray-400 italic">{t("summary.noDateSelected")}</p>
                    )}
                  ...
                  {/* Children Summary */}
                  <div className="pb-4 border-b">
                    <p className="text-sm text-gray-500 mb-2">{t("summary.childrenLabel")}</p>
                        ...
                        {bookingDetails.selectedChildren
                          ...
                          .map((child) => (
                            <li key={child.id} className="text-sm">
                              • {t("summary.childAgeShort", { name: child.name, age: child.age })}
                            </li>
                          ))}
                      </ul>
                    ) : (
                      <p className="text-gray-400 italic">
                        {t("summary.noChildrenSelected")}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">{t("summary.hourlyRateLabel")}</span>
                      <span>{t("summary.hourlyRateValue", { rate: selectedSitter?.hourly_rate })}</span>
                    </div>
                    {bookingDetails.startTime && bookingDetails.endTime && (
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-600">{t("summary.durationLabel")}</span>
                        <span>
                          {(() => {
                    ...
                    {selectedAdventureRecords.length > 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-600">{t("summary.adventuresPriceLabel")}</span>
                        <span>
                          {t("summary.adventuresTotal", {
                            total: selectedAdventureRecords
                              .reduce(
                                (sum, adventure) =>
                    ...
                    <div className="flex justify-between font-bold text-lg pt-2 border-t">
                      ...
                      <span className="text-[#ff6b6b]">
                        {t("summary.totalValue", { total: calculateTotal() })}
                      </span>
                    </div>
                  </div>
