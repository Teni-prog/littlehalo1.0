...
const STATUS_FILTER_KEYS = [
  ...
  { key: "pending_sitter", labelKey: "pending" },
  { key: "confirmed",      labelKey: "confirmed" },
  { key: "completed",      labelKey: "completed" },
  { key: "declined",       labelKey: "declined" },
];

const STATUS_BADGE_KEYS = {
  confirmed:      { labelKey: "confirmed",      cls: "bg-teal-50 text-teal-600" },
  pending_sitter: { labelKey: "awaitingSitter",  cls: "bg-yellow-50 text-yellow-700" },
  declined:       { labelKey: "declined",        cls: "bg-red-50 text-red-500" },
  cancelled:      { labelKey: "cancelled",       cls: "bg-gray-100 text-gray-500" },
  completed:      { labelKey: "completed",       cls: "bg-purple-50 text-purple-600" },
};

export default function AllBookingsPage() {
  const t = useTranslations("bookingsParent");
        ...
        if (bookingRes.ok) {
          ...
          const mapped = bookings
            .map((b) => {
              ...
              const sitterName   = b.sitter_profile?.user?.name || t("fallback.yourSitter");
              ...
              return {
                ...
                child:  firstChild ? `${firstChild.name} (${firstChild.age})` : t("fallback.child"),
                adventure: b.adventure_id || t("fallback.notSpecified"),
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-8 py-12 text-center">
            <p className="text-gray-400 text-sm">
              {sessions.length === 0 ? (
                <>{t("empty.noBookingsYet")}{" "}<a href="/search" className="text-[#ff6b6b] hover:underline">{t("empty.findASitter")}</a></>
              ) : (
                t("empty.noBookingsMatchFilter")
              )}
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {filtered.map((session) => {
              const badgeConfig = STATUS_BADGE_KEYS[session.status];
              const badge = badgeConfig
                ? { label: t(`statusBadge.${badgeConfig.labelKey}`), cls: badgeConfig.cls }
                : { label: session.status, cls: "bg-gray-100 text-gray-500" };
              return (
                <div
                  key={session.id}
                ...
                >
                  ...
                  <p className="text-sm text-gray-500 mb-3">
                    {t("session.for")} <span className="font-semibold text-gray-800">{session.child}</span>
                  </p>

                  {/* Micro-adventure */}
                  <div className="bg-yellow-50 text-yellow-800 px-4 py-2.5 rounded-xl text-sm font-medium border border-yellow-100">
                    <span className="font-bold">{t("session.microAdventure")}</span> {session.adventure}
                  </div>
                      ...
                      {session.parentMarkedComplete ? (
                        ...
                        >
                          {markingComplete === session.id
                            ? t("markComplete.marking")
                            : t("markComplete.markComplete")}
                        </button>
                      )}
                    </div>
                  )}
