...
function AvailabilityList({ availability }) {
  ...
  const rows = DAYS.flatMap(({ key }) => {
    ...
    return [{ label: t(`days.${key}`), range: `${start} – ${end}` }];
  });
  ...
  useEffect(() => {
    ...
    async function fetchSitter() {
      try {
        ...
        if (!response.ok) { setError(result.error || t("errors.fetchFailed")); return; }
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-4">
                ...
                <div className="text-center md:text-right">
                  <div className="text-3xl font-bold text-[#ff6b6b] mb-1">
                    ${sitter.hourly_rate}
                    <span className="text-lg text-gray-600">{t("perHourShort")}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="font-semibold">
                      {t("stats.experienceYears", { count: sitter.experience || 0 })}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm text-gray-600">{t("stats.reviews")}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6">
              <h2 className="text-xl font-bold mb-4">{t("sections.aboutMe")}</h2>
            ...
            {sitter.languages && sitter.languages.length > 0 && (
              <div className="bg-white rounded-2xl shadow-sm p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="text-xl font-bold">{t("sections.languages")}</h2>
                </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            {sitter.latitude && sitter.longitude && (
              <div className="bg-white rounded-2xl shadow-sm p-6">
                <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                  <MapPin className="w-5 h-5 text-[#ff6b6b]" /> {t("sections.location")}
                </h2>
                <NeighbourhoodMap
            ...
            {/* Availability */}
            <div className="bg-white rounded-2xl shadow-sm p-6">
              <h2 className="text-xl font-bold mb-4">{t("sections.availability")}</h2>
              <AvailabilityList availability={sitter.recurring_availability} />
            </div>

            {/* Contact Card */}
            <div className="bg-white rounded-2xl shadow-sm p-6">
              <h2 className="text-xl font-bold mb-4">{t("sections.contact")}</h2>
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-gray-700">
                  <DollarSign className="w-5 h-5 text-[#ff6b6b]" />
                  <span>${sitter.hourly_rate}{t("perHour")}</span>
                </div>
                <div className="flex items-center gap-3 text-gray-700">
            ...
            {viewerRole !== "sitter" && (
              <div className="bg-gradient-to-br from-[#ff6b6b]/10 to-teal-50 rounded-2xl shadow-sm p-6">
                <h3 className="font-bold mb-3">{t("readyToBook")}</h3>
                <p className="text-sm text-gray-600 mb-4">
                  {t("scheduleSessionWith", { firstName: sitter.name.split(" ")[0] })}
                </p>
                <Link href={`/booking?sitterId=${sitter.id}`}>
