...
import { updateParentPreferences } from "@/app/[locale]/actions/parent";

export default function ParentPreferences({ parent, onUpdate }) {
  const t = useTranslations("profileParentPreferences");
    ...
    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sm:p-8 mb-8">
      <div className="flex justify-between items-center mb-6">
        ...
        {!editing ? (
          ...
          >
            <Pencil className="w-4 h-4" /> {t("editPreferences")}
          </button>
        ) : null}
      </div>
      <hr className="mb-6" />
      ...
      {!editing ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="font-semibold text-gray-900 mb-1">{t("view.budget")}</h3>
              <p className="text-gray-600">{t("view.budgetValue", { amount: parent.max_budget })}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="font-semibold text-gray-900 mb-1">{t("view.languages")}</h3>
              <div className="flex flex-wrap gap-1">
                {parent.preferred_languages?.length ? (
                  ...
                  <p className="text-gray-600">{t("view.any")}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="font-semibold text-gray-900 mb-1">{t("view.location")}</h3>
              <p className="text-gray-600">{parent.preferred_location || t("view.notSet")}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="font-semibold text-gray-900 mb-1">{t("view.specialNeeds")}</h3>
              <p className="text-gray-600">{parent.special_needs || t("view.noneNoted")}</p>
            </div>
          </div>
        </div>
      ) : (
        ...
        <form action={formAction} className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700 flex items-center gap-1.5">
                <MapPin className="w-4 h-4 text-teal-500" /> {t("form.location")}
              </label>
              <input
                name="location" type="text"
                value={location} onChange={e => setLocation(e.target.value)}
                placeholder={t("form.locationPlaceholder")} required
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700 flex items-center gap-1.5">
                <DollarSign className="w-4 h-4 text-purple-500" /> {t("form.maxBudget")}
              </label>
              <input
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-medium text-gray-700 flex items-center gap-1.5">
              <Languages className="w-4 h-4 text-blue-500" /> {t("form.preferredLanguages")}
            </label>
            <input
              type="text"
              value={languagesText}
              ...
              className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:border-[#ff6b6b] focus:ring-2 focus:ring-[#ff6b6b]/20 transition-all"
            />
            <p className="text-xs text-gray-400">{t("form.languagesHint")}</p>
            {parsedLanguages.length === 0 && (
              <p className="text-xs text-red-500">{t("form.languagesError")}</p>
            )}
          </div>
