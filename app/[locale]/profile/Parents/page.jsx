...
export default function ParentDashboard() {
  const t = useTranslations("profileParent");
        ...
        setSessions(
          bookings.map((b) => {
            ...
            return {
              id:     b.id,
              status: b.status,
              date:   new Date(b.date).toLocaleDateString(dateLocale, { month: "short", day: "numeric", year: "numeric" }),
  ...
  const actions = [
    {
      icon: Search,
      label: t("quickActions.findSitters.label"),
      desc: t("quickActions.findSitters.desc"),
      color: "bg-red-50 text-[#ff6b6b]",
      href: "/search",
    },
    {
      icon: BookOpen,
      label: t("quickActions.adventures.label"),
      desc: t("quickActions.adventures.desc"),
    ...
    {
      icon: CreditCard,
      label: t("quickActions.payments.label"),
      desc: t("quickActions.payments.desc"),
      color: "bg-gray-100 text-gray-400",
      comingSoon: true,
    },
  ];
  ...
  const getStatusBadge = (status) => {
    const map = {
      confirmed:      { label: t("bookings.statusBadges.confirmed"),      cls: "bg-teal-50 text-teal-600" },
      pending_sitter: { label: t("bookings.statusBadges.awaitingSitter"), cls: "bg-yellow-50 text-yellow-700" },
      declined:       { label: t("bookings.statusBadges.declined"),       cls: "bg-red-50 text-red-500" },
      cancelled:      { label: t("bookings.statusBadges.cancelled"),      cls: "bg-gray-100 text-gray-500" },
      completed:      { label: t("bookings.statusBadges.completed"),      cls: "bg-purple-50 text-purple-600" },
    };
  ...
  const getStatusMessage = (status, sitterName) => {
    if (status === "pending_sitter")
      return t("bookings.statusMessages.pendingSitter", { sitterName });
    if (status === "confirmed")
      return t("bookings.statusMessages.confirmed", { sitterName });
    if (status === "declined")
      return t("bookings.statusMessages.declined", { sitterName });
    return null;
  };
        ...
        <div className="flex flex-col min-h-screen max-w-7xl mx-auto px-4 sm:px-6 py-8">
            ...
            {actions.map((action, index) =>
              action.comingSoon ? (
                ...
                >
                  ...
                  <p className="text-xs text-gray-400">{t("quickActions.comingSoon")}</p>
                </div>
              ) : (
                <Link
                  key={index}
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm mb-8 overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3 flex-wrap">
                <h2 className="text-xl font-bold text-gray-900 text-left">
                  {t("matchPriorities.title")}
                </h2>
                {usingLearnedWeights && (
                  <span className="text-xs bg-green-100 text-green-700 px-2.5 py-1 rounded-full font-medium">
                    {t("matchPriorities.learnedWeightsActive")}
                  </span>
                )}
                <span className="text-xs text-gray-400">
                  {t("matchPriorities.subtitle")}
                </span>
              </div>
              <span className="text-gray-400 text-sm ml-4 flex-shrink-0">
                {showWeightConfig ? t("matchPriorities.hide") : t("matchPriorities.show")}
              </span>
            </button>
            ...
            {showWeightConfig && (
              <div className="px-6 pb-8 sm:px-8 border-t border-gray-100 pt-6">
                ...
                >
                  {t("matchPriorities.resetToDefaults")}
                </button>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sm:p-8 mb-8">
            ...
            <div className="flex items-center gap-2 mt-4 mb-1 flex-wrap">
              <span className="text-xs font-semibold text-gray-400 mr-1">{t("topMatches.filterByDistance")}</span>
              {[1, 3, 5, 10].map((km) => (
                <button
                  key={km}
              ...
              <div className="lg:col-span-3 flex flex-col gap-4">
                {sittersForDisplay.length > 0 ? (
                  ...
                  <p className="text-gray-500 text-center py-8 text-sm">
                    {t("topMatches.noneWithinRadius", { radiusKm })}
                  </p>
                ) : (
                  <p className="text-gray-500 text-center py-8">
                    {t("topMatches.noneFound")}
                  </p>
                )}
              </div>
              ...
              <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sm:p-8">
                ...
                <div className="space-y-6 mt-4">
                  {loadingSessions ? (
                    ...
                    <p className="text-center text-gray-400 py-8 text-sm">{t("bookings.empty")} <a href="/search" className="text-[#ff6b6b] hover:underline">{t("bookings.findASitter")}</a></p>
                  ) : null}
                      ...
                      <div key={session.id} className="border border-gray-100 rounded-2xl p-6 bg-gray-50/50">
                        ...
                        <div className="mb-4">
                          <span className="text-sm text-gray-500">
                            {t("bookings.forChild")} <span className="font-semibold text-gray-900">{session.child}</span>
                          </span>
                        </div>

                        <div className="bg-yellow-50 text-yellow-800 px-4 py-3 rounded-xl text-sm font-medium border border-yellow-100">
                          <span className="font-bold">{t("bookings.microAdventure")}</span> {session.adventure}
                        </div>
                          ...
                          <div className="mt-4">
                            {session.parentMarkedComplete ? (
                              <p className="text-sm text-center text-gray-500 py-2 bg-gray-50 rounded-xl border border-gray-100">
                                {t("bookings.markedCompleteWaiting")}
                              </p>
                            ) : (
                              <button
                              ...
                              >
                                {markingComplete === session.id ? t("bookings.marking") : t("bookings.markComplete")}
                              </button>
                            )}
                          </div>
                        )}
                            ...
                            {reviewedSet.has(session.id) ? (
                              <div className="flex items-center justify-between gap-3 py-2.5 px-3 bg-teal-50 rounded-xl border border-teal-100">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-sm text-teal-600 font-medium">{t("bookings.reviewSubmitted")}</span>
                                </div>
