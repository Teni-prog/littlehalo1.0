...
import { completeSitterSignup } from "@/app/actions/auth";
import {
    ...
    <div className="space-y-5">
      ...
      <Field label="Phone Number" error={errors.phone} required>
        <TextInput icon={Phone} type="tel" value={data.phone} error={errors.phone}
          onChange={e => update("phone", e.target.value)} placeholder="(506) 555-0100" />
      </Field>

      <Field label="Area" error={errors.neighbourhood} required>
        <div className="relative">
        ...
        {data.neighbourhood && (
          <p className="text-xs text-teal-600 mt-1 flex items-center gap-1">
            <MapPin className="w-3 h-3" /> Location set to {data.neighbourhood} area
          </p>
        )}
      </Field>
      ...
      <Field label="Country of Origin" error={errors.country} required>
        <SearchableDropdown
          options={COUNTRIES} value={data.country} error={errors.country}
          onChange={v => update("country", v)} placeholder="Search country…" />
      </Field>
      ...
      <Field label="Short Bio">
        <div className="relative">
          ...
          <textarea
            ...
            placeholder="Tell families about yourself, your background, and childcare experience…"
            rows={4}
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        ...
        <Field label="Hourly Rate (CAD)" error={errors.hourlyRate} required>
          <TextInput icon={DollarSign} type="number" value={data.hourlyRate} error={errors.hourlyRate}
            onChange={e => update("hourlyRate", e.target.value)}
            placeholder="e.g. 20" min={10} max={200} />
        </Field>
      </div>

      <Field label="Age Groups" error={errors.ageGroups} required>
        <TagSelect options={AGE_GROUPS} selected={data.ageGroups}
          onChange={v => update("ageGroups", v)} />
      </Field>

      <Field label="Special Needs Experience">
...
export default function SitterSignupPage() {
  ...
  function validate2() {
    ...
    if (!data.neighbourhood)       e.neighbourhood = "Select your neighbourhood.";
    if (data.languages.length < 1) e.languages    = "Select at least one language.";
    if (!data.country)             e.country      = "Country of origin is required.";
    setErrors(e);
    return Object.keys(e).length === 0;
  }
  ...
  function validate3() {
    const e = {};
    if (data.experience === "")  e.experience = "Enter years of experience (0 is fine).";
    if (data.ageGroups.length < 1) e.ageGroups = "Select at least one age group.";
  ...
  const stepTitles = ["Basic Account", "Personal Info", "Caregiver Credentials", "Availability"];
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
          ...
          <div className="flex gap-3 mt-8">
            ...
            {step === 4 && (
              ...
              >
                {loading ? "Saving profile…" : "Complete Profile"} <Check className="w-4 h-4" />
              </button>
            )}
          </div>
        </div>
