  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <Star
          key={n}
          className={`${cls} ${n <= value ? "fill-amber-400 text-amber-400" : "fill-gray-200 text-gray-200"}`}
        />
      ))}
    </div>
  );
...
export default function SitterReviews({ sitterId }) {
  ...
  const { reviews = [], overall_rating, avg_session_rating, avg_adventure_rating, total_reviews = 0 } = data || {};
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        {overall_rating != null && (
          <div className="flex items-center gap-1.5 text-sm text-gray-600">
            ...
            <span className="text-gray-400">({total_reviews} {total_reviews === 1 ? "review" : "reviews"})</span>
          </div>
        )}
      </div>
      ...
      {reviews.length === 0 ? (
        ...
        <div className="space-y-5">
          {reviews.map((review, idx) => (
            <div key={review.id} className={`pb-5 ${idx < reviews.length - 1 ? "border-b border-gray-100" : ""}`}>
              <div className="flex items-start gap-3">
                {/* Avatar */}
                <div
                  className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-xs shrink-0 ${avatarColor(review.reviewer_initials)}`}
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* Text */}
                  {review.review_text && (
                    <p className="text-sm text-gray-600 leading-relaxed mt-2">{review.review_text}</p>
                  )}
                </div>
              </div>
            </div>
